...
export const BUILDINGS: Record<BuildingType, BuildingDef> = {
  ...
  farm: {
    ...
    baseCost: { money: 160, materials: 100 },
    baseBuildTicks: 90,
  ...
  commercial: {
    ...
    baseCost: { money: 260, materials: 130 },
    baseBuildTicks: 120,
  ...
  factory: {
    ...
    baseCost: { money: 300, materials: 180 },
    baseBuildTicks: 150,
  ...
  mine: {
    ...
    baseCost: { money: 210, materials: 160 },
    baseBuildTicks: 130,
  ...
  power_plant: {
    ...
    baseCost: { money: 360, materials: 240, oil: 65 },
    baseBuildTicks: 180,
  ...
  barracks: {
    ...
    baseCost: { money: 200, materials: 150 },
    baseBuildTicks: 100,
    defenceBonus: 0.05,
  ...
  academy: {
    ...
    baseCost: { money: 400, materials: 210, research: 35 },
    baseBuildTicks: 200,
    defenceBonus: 0.03,
  ...
  vehicle_plant: {
    ...
    baseCost: { money: 500, materials: 340, oil: 48 },
    baseBuildTicks: 240,
    unlocks: ['tank', 'apc'],
    mvp: false,
  },
  airbase: {
    ...
    baseCost: { money: 560, materials: 300, oil: 110 },
    baseBuildTicks: 260,
  ...
  naval_base: {
    ...
    baseCost: { money: 540, materials: 370, oil: 95 },
    baseBuildTicks: 260,
  ...
  university: {
    ...
    baseCost: { money: 420, materials: 190 },
    baseBuildTicks: 200,
  ...
  advanced_lab: {
    ...
    baseCost: { money: 720, materials: 400, research: 100 },
    baseBuildTicks: 300,
...
export const MVP_BUILDINGS: BuildingDef[] = BUILDING_LIST.filter((b) => b.mvp);

/** Cost of taking a building from `level-1` to `level` (level 1 = initial construction). */
export function buildingCost(type: BuildingType, level: number): ResourceDelta {
  const def = BUILDINGS[type];
  const mul = Math.pow(COST_GROWTH, level - 1);
  const out: ResourceDelta = {};
  for (const [k, v] of Object.entries(def.baseCost)) {
    out[k as keyof ResourceDelta] = Math.round((v as number) * mul);
  }
  return out;
}

/** Ticks required to reach `level`, before any build-speed modifiers. */
export function buildingTicks(type: BuildingType, level: number): number {
...
export function buildingYield(type: BuildingType, level: number): ResourceDelta {
  const def = BUILDINGS[type];
  if (!def.yieldPerMinute) return {};
  const out: ResourceDelta = {};
  for (const [k, v] of Object.entries(def.yieldPerMinute)) {
    out[k as keyof ResourceDelta] = (v as number) * level;
  }
  return out;
}

/** Human-readable tier name for the level, e.g. farm level 9 → "AI Agriculture". */
export function tierName(type: BuildingType, level: number): string {
  const t = BUILDINGS[type].tiers;
  if (level <= 3) return t[0];
