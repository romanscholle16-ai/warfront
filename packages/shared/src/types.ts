...
export const RESOURCE_KEYS = ['money', 'food', 'oil', 'materials', 'research'] as const;
...
export type Terrain =
  | 'city'      // income
  | 'plains'    // balanced, best farmland
  | 'mountain'  // defence
  | 'coast'     // naval advantage
  | 'desert'    // oil
  | 'forest'    // materials
  | 'tundra';   // sparse
...
export interface TerritoryDef {
  ...
  resources: { oil: number; materials: number; food: number };
...
export type BuildingType =
  ...
  // military
  | 'barracks' | 'academy' | 'vehicle_plant' | 'airbase' | 'naval_base'
  // technology
...
export interface Player {
  ...
  /** Completed tech ids. */
  tech: string[];
  research: { techId: string; completesAtTick: number } | null;
...
export type GameEventType =
  | 'match_started' | 'territory_captured' | 'battle_started' | 'battle_ended'
  | 'building_completed' | 'units_trained' | 'research_completed'
  | 'player_eliminated' | 'player_disconnected' | 'player_reconnected'
  | 'alliance_formed' | 'alliance_offered' | 'war_declared' | 'betrayal'
  | 'trade_offered' | 'trade_completed' | 'trade_declined'
  | 'leader_level_up' | 'ping' | 'chat'
  | 'match_ended';
