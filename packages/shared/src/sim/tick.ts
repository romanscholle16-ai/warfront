...
function accrueXp(state: MatchState): void {
  ...
  for (const player of Object.values(state.players)) {
    ...
    if (held > 0) awardMatchXp(state, player.id, held * XP_REWARDS.perMinutePerTerritory);
  }
}

/** Runs `n` ticks. Used by tests, the headless balance tool and fast-forward. */
export function tickMany(state: MatchState, n: number): void {
 ...
 */
function applyAttrition(state: MatchState, bankrupt: Set<string>): void {
  if (bankrupt.size === 0) return;
  const lossPerMinute = 0.06;
  for (const army of Object.values(state.armies)) {
    if (!bankrupt.has(army.ownerId)) continue;
    for (const [unitId, count] of Object.entries(army.units)) {
      if (!count) continue;
      const key = unitId as keyof typeof UNITS;
...
function checkElimination(state: MatchState): void {
  for (const player of Object.values(state.players)) {
    if (player.eliminatedAtTick !== null) continue;
    if (ownedTerritoryCount(state, player.id) > 0) continue;
...
function checkVictory(state: MatchState): void {
  ...
  for (const player of Object.values(state.players)) {
    if (player.eliminatedAtTick === null) aliveTeams.add(player.team);
    const held = ownedTerritoryCount(state, player.id);
...
function endMatch(state: MatchState, winnerTeam: number | null, reason: string): void {
  state.phase = 'ended';
  ...
  const winners = Object.values(state.players)
    ...
    .map((p) => p.name);
  addEvent(state, 'match_ended', null,
    winnerTeam === null ? 'The world lies in ruins. No victor.' : `Victory: ${winners.join(', ')}.`,
    { data: { winnerTeam, reason } });
}
