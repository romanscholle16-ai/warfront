 ...
 */
export function canCrossLink(units: UnitCounts, kind: LinkKind): boolean {
...
export function traverseBlocker(
  units: UnitCounts,
  kind: LinkKind,
  destinationCoastal: boolean,
): TraverseBlocker | null {
  for (const [unitId, count] of Object.entries(units)) {
    if (!count) continue;
    const def = UNITS[unitId as keyof typeof UNITS];
    if (!def) continue;

    if (def.domain === 'air') continue; // aircraft ignore terrain entirely

    if (def.domain === 'sea') {
      if (kind !== 'sea') return 'ships_need_water';
      if (!destinationCoastal) return 'ships_need_coast';
      continue;
    }

    // Land units.
    if (kind === 'sea' && def.id !== 'marine') return 'cannot_cross_sea';
  }
  return null;
}

/** True if the army contains anything that can hold ground — ships and planes cannot. */
export function hasGroundForce(units: UnitCounts): boolean {
  for (const [unitId, count] of Object.entries(units)) {
    if (!count || count < 0.01) continue;
    if (UNITS[unitId as keyof typeof UNITS]?.domain === 'land') return true;
  }
  return false;
}

/** An army moves at the speed of its slowest unit. */
export function armySpeed(state: MatchState, army: Army): number {
  let slowest = Infinity;
  for (const [unitId, count] of Object.entries(army.units)) {
    if (!count) continue;
    const def = UNITS[unitId as keyof typeof UNITS];
    if (def) slowest = Math.min(slowest, def.speed);
  }
...
export function travelTicks(state: MatchState, army: Army, to: string): number {
...
export function moveBlocker(state: MatchState, army: Army, to: string): MoveBlocker | null {
  if (armyUnitCount(army) <= 0) return 'empty_army';
  if (army.movingTo !== null) return 'already_moving';
  const link = linkBetween(army.at, to);
  if (!link) return 'not_adjacent';
...
export function legalDestinations(state: MatchState, army: Army): string[] {
 ...
 */
export function routeForArmy(state: MatchState, army: Army, destination: string): string[] | null {
  if (!state.territories[destination] || army.at === destination) return null;
  const queue: string[] = [army.at];
  ...
  while (queue.length) {
    ...
    for (const link of ADJACENCY[from] ?? []) {
      ...
      if (traverseBlocker(army.units, link.kind, TERRITORY_DEFS[link.to]?.coastal ?? false)) continue;
      // Only the final step can pass through a country we do not control.
      if (link.to !== destination && state.territories[link.to]?.ownerId !== army.ownerId) continue;
      visited.add(link.to);
...
export function orderMove(state: MatchState, army: Army, to: string): void {
  army.movingTo = to;
  army.progress = 0;
}
 ...
 */
export function tickArmies(state: MatchState): void {
  const arrived: Army[] = [];

  for (const army of Object.values(state.armies)) {
    if (army.movingTo === null) continue;
...
function mergeIntoGarrison(state: MatchState, army: Army): void {
  for (const other of Object.values(state.armies)) {
    if (other.id === army.id) continue;
    if (other.ownerId !== army.ownerId || other.at !== army.at || other.movingTo !== null) continue;
...
export function cullEmptyArmies(state: MatchState): void {
  for (const [id, army] of Object.entries(state.armies)) {
    for (const [unitId, count] of Object.entries(army.units)) {
      if ((count ?? 0) < 0.01) delete army.units[unitId as keyof UnitCounts];
    }
    if (armyUnitCount(army) <= 0) delete state.armies[id];
  }
}

/** Splits `units` out of `army` into a new army standing in the same territory. */
export function splitArmy(state: MatchState, army: Army, units: UnitCounts, newId: string): Army | null {
  const taken: UnitCounts = {};
  for (const [unitId, requested] of Object.entries(units)) {
    const key = unitId as keyof UnitCounts;
  ...
  let total = 0;
  for (const n of Object.values(taken)) total += n ?? 0;
