...
export function allyBlocker(state: MatchState, fromId: string, toId: string): AllyBlocker | null {
  if (fromId === toId) return 'self';
  const from = state.players[fromId];
  const to = state.players[toId];
  if (!from || !to) return 'unknown_player';
  if (from.allies.includes(toId)) return 'already_allied';
  if (to.allianceOffers.includes(fromId)) return 'already_offered';
  return null;
}
 ...
 */
export function proposeAlliance(state: MatchState, fromId: string, toId: string): 'formed' | 'offered' {
  const from = state.players[fromId]!;
  const to = state.players[toId]!;
  ...
  if (!to.allianceOffers.includes(fromId)) to.allianceOffers.push(fromId);
  addEvent(state, 'alliance_offered', toId, `${from.name} proposes an alliance.`,
    { data: { from: fromId } });
  return 'offered';
}

/** `accepterId` accepts the offer standing from `proposerId`. */
export function acceptAlliance(state: MatchState, accepterId: string, proposerId: string): boolean {
  const accepter = state.players[accepterId];
  const proposer = state.players[proposerId];
  if (!accepter || !proposer) return false;
  if (!accepter.allianceOffers.includes(proposerId)) return false;
  ...
  if (!accepter.allies.includes(proposerId)) accepter.allies.push(proposerId);
  if (!proposer.allies.includes(accepterId)) proposer.allies.push(accepterId);

  addEvent(state, 'alliance_formed', null,
    `${proposer.name} and ${accepter.name} have formed an alliance.`,
    { data: { a: proposerId, b: accepterId } });
  return true;
}

export function declineAlliance(state: MatchState, declinerId: string, proposerId: string): void {
 ...
 */
export function breakAlliance(state: MatchState, breakerId: string, targetId: string): boolean {
  const breaker = state.players[breakerId];
  const target = state.players[targetId];
  if (!breaker || !target) return false;
  if (!breaker.allies.includes(targetId)) return false;
  ...
  addEvent(state, 'betrayal', null,
    `${breaker.name} has betrayed ${target.name}. Their people are uneasy.`,
    { data: { breaker: breakerId, target: targetId } });
  return true;
}
...
export type TradeBlocker =
  | 'unknown_player' | 'self' | 'empty_offer' | 'too_many_offers' | 'insufficient_resources';
...
export function tradeBlocker(
  ...
  if (fromId === toId) return 'self';
  const from = state.players[fromId];
  if (!from || !state.players[toId]) return 'unknown_player';

  const giveTotal = sumDelta(give);
  const wantTotal = sumDelta(want);
  if (giveTotal <= 0 && wantTotal <= 0) return 'empty_offer';
  if (!canAfford(from.resources, give)) return 'insufficient_resources';

  const open = state.tradeOffers.filter((offer) => offer.fromId === fromId).length;
  if (open >= MAX_OPEN_OFFERS_PER_PLAYER) return 'too_many_offers';
  return null;
}
...
export function createTradeOffer(
  ...
  addEvent(state, 'trade_offered', toId,
    `${from.name} offers you a trade.`, { data: { offerId: offer.id, from: fromId } });
  return offer;
}

export type AcceptTradeResult = 'accepted' | 'not_found' | 'not_recipient' | 'cannot_afford';

export function acceptTradeOffer(state: MatchState, accepterId: string, offerId: string): AcceptTradeResult {
  const index = state.tradeOffers.findIndex((offer) => offer.id === offerId);
  if (index < 0) return 'not_found';
  const offer = state.tradeOffers[index]!;
  if (offer.toId !== accepterId) return 'not_recipient';

  const accepter = state.players[accepterId];
  const proposer = state.players[offer.fromId];
  if (!accepter || !proposer) return 'not_found';
  if (!canAfford(accepter.resources, offer.want)) return 'cannot_afford';
  ...
  for (const key of RESOURCE_KEYS) {
    ...
    if (value !== undefined) accepter.resources[key] += value * rate;
  }

  state.tradeOffers.splice(index, 1);
  addEvent(state, 'trade_completed', null,
    `${proposer.name} and ${accepter.name} completed a trade.`,
    { data: { from: offer.fromId, to: accepterId, give: offer.give, want: offer.want } });
  return 'accepted';
}

/** Declining or expiring both return the escrowed goods. */
export function cancelTradeOffer(state: MatchState, offerId: string, reason: 'declined' | 'expired'): void {
  const index = state.tradeOffers.findIndex((offer) => offer.id === offerId);
  if (index < 0) return;
  const offer = state.tradeOffers[index]!;
  const proposer = state.players[offer.fromId];
  if (proposer) refund(proposer.resources, offer.give);
  state.tradeOffers.splice(index, 1);
  addEvent(state, 'trade_declined', offer.fromId,
    reason === 'expired' ? 'A trade offer expired.' : 'Your trade offer was declined.',
    { data: { offerId } });
}

export function expireTradeOffers(state: MatchState): void {
  for (let i = state.tradeOffers.length - 1; i >= 0; i--) {
    const offer = state.tradeOffers[i]!;
    if (state.tick >= offer.expiresAtTick) cancelTradeOffer(state, offer.id, 'expired');
  }
}

function sumDelta(delta: ResourceDelta): number {
  let total = 0;
  for (const key of RESOURCE_KEYS) total += delta[key] ?? 0;
  return total;
}

/** Rejects negative or absurd values before they ever reach the economy. */
export function sanitizeDelta(delta: ResourceDelta): ResourceDelta {
...
export function addPing(state: MatchState, playerId: string, territoryId: string, kind: PingKind): void {
...
export function expirePings(state: MatchState): void {
  if (state.pings.length === 0) return;
