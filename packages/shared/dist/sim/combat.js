...
import { CAPTURE_TICKS, CAPTURE_UNREST, COMBAT_DAMAGE_RATE, COMBAT_JITTER, TERRAIN_DEFENCE_MUL, } from '../config/constants.js';
...
function partitionIntoSides(state, armies) {
    const sides = [];
    for (const army of armies) {
        ...
        for (const side of sides) {
            if (side.ownerIds.every((id) => areFriendly(state, id, army.ownerId))) {
                ...
                if (!side.ownerIds.includes(army.ownerId))
                    side.ownerIds.push(army.ownerId);
                placed = true;
                break;
            }
        }
        if (!placed)
            sides.push({ ownerIds: [army.ownerId], armies: [army], attack: 0, defence: 0, hp: 0 });
    }
    return sides;
}
function dominantDomain(side) {
    const totals = { land: 0, air: 0, sea: 0 };
    for (const army of side.armies) {
        for (const [unitId, count] of Object.entries(army.units)) {
            const def = UNITS[unitId];
            if (def && count)
                totals[def.domain] += count;
        }
    }
    if (totals.air >= totals.land && totals.air >= totals.sea)
        return 'air';
    if (totals.sea >= totals.land)
        return 'sea';
    return 'land';
}
function scoreSide(state, side, territoryId, enemyDomain) {
...
function fight(state, territoryId, sides) {
    ...
    sides.forEach((side, i) => {
        ...
        for (let j = 0; j < sides.length; j++)
            if (j !== i)
                incoming += damage[j];
        if (incoming > 0)
            applyCasualties(state, territoryId, side, incoming);
    });
}
/** Spreads `damage` (in HP) across a side's units, proportional to each type's share of HP. */
function applyCasualties(state, territoryId, side, damage) {
    if (side.hp <= 0)
        return;
    ...
    for (const army of side.armies) {
        for (const [unitId, count] of Object.entries(army.units)) {
            ...
            if (battle)
                battle.losses[army.ownerId] = (battle.losses[army.ownerId] ?? 0) + lost;
        }
    }
}
function strongestOwner(state, side) {
...
function capture(state, territoryId, newOwnerId) {
    const territory = state.territories[territoryId];
    ...
    if (previous)
        awardMatchXp(state, previous, XP_REWARDS.territoryLost);
    const name = territoryDef(territoryId).name;
    const takerName = state.players[newOwnerId]?.name ?? 'Someone';
    addEvent(state, 'territory_captured', newOwnerId, previous ? `${takerName} has taken ${name}.` : `${takerName} has occupied neutral ${name}.`, { territoryId, data: { from: previous } });
    if (previous) {
        addEvent(state, 'territory_captured', previous, `You have lost ${name}.`, { territoryId });
    }
}
function ensureBattleRecord(state, territoryId, sides) {
    ...
    addEvent(state, 'battle_started', null, `Fighting has broken out in ${territoryDef(territoryId).name}.`, { territoryId });
}
function closeFinishedBattles(state, contested) {
    for (let i = state.battles.length - 1; i >= 0; i--) {
        const battle = state.battles[i];
        if (contested.has(battle.territoryId))
            continue;
        addEvent(state, 'battle_ended', null, `The battle for ${territoryDef(battle.territoryId).name} is over.`, { territoryId: battle.territoryId, data: { losses: battle.losses } });
