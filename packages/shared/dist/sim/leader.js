 ...
 */
export const SKILL_BRANCHES = ['warfare', 'economy', 'intelligence'];
...
export function grantXp(leader, amount) {
    if (amount <= 0)
...
export function skillBlocker(leader, branch) {
    if (!SKILL_BRANCHES.includes(branch))
        return 'unknown_branch';
    if (leader.skillPoints < 1)
        return 'no_points';
    if (leader.skills[branch] >= MAX_SKILL_POINTS_PER_BRANCH)
        return 'branch_maxed';
    return null;
}
export function spendSkillPoint(leader, branch) {
    if (skillBlocker(leader, branch) !== null)
        return false;
...
export function awardMatchXp(state, playerId, amount) {
 ...
 */
export function finaliseMatchXp(state) {
    for (const player of Object.values(state.players)) {
        ...
        if (levelsGained > 0) {
            addEvent(state, 'leader_level_up', player.id, `${player.leader.name} reached level ${player.leader.level} (+${levelsGained} skill point${levelsGained > 1 ? 's' : ''}).`, { data: { level: player.leader.level, skillPoints: player.leader.skillPoints } });
        }
