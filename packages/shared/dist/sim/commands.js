...
import { addEvent, canAfford, missingResources, nextId, refund, spend, } from './state.js';
import { beginBuilding, beginTraining, buildingBlocker, nextBuildingLevel, unitBlocker, } from './construction.js';
...
import { acceptAlliance, acceptTradeOffer, addPing, allyBlocker, breakAlliance, cancelTradeOffer, createTradeOffer, declineAlliance, proposeAlliance, sanitizeDelta, tradeBlocker, } from './diplomacy.js';
const OK = { ok: true };
...
export function commandCost(state, cmd) {
    switch (cmd.t) {
        case 'BUILD': {
 ...
 */
export function validateCommand(state, playerId, cmd) {
    ...
    switch (cmd.t) {
        ...
        case 'CANCEL_BUILD': {
            ...
            if (!b || b.completesAtTick === 0)
                return fail('nothing_in_progress');
            return OK;
        }
        case 'TRAIN': {
            if (!Number.isInteger(cmd.count) || cmd.count < 1 || cmd.count > 50)
        ...
        case 'SPLIT_ARMY': {
            ...
            if (army.movingTo !== null)
                return fail('already_moving');
            let requested = 0;
            for (const [unitId, n] of Object.entries(cmd.units)) {
                const have = army.units[unitId] ?? 0;
                if ((n ?? 0) > have)
                    return fail('not_enough_units');
                requested += n ?? 0;
            }
            if (requested <= 0)
                return fail('empty_split');
        ...
        case 'START_RESEARCH': {
            const tech = TECH_BY_ID[cmd.techId];
            if (!tech)
                return fail('unknown_tech');
            if (player.research)
                return fail('already_researching');
            if (player.tech.includes(cmd.techId))
                return fail('already_known');
            const prereq = techPrerequisite(cmd.techId);
            if (prereq && !player.tech.includes(prereq))
                return fail('missing_prerequisite');
            return OK;
        }
        case 'SPEND_SKILL': {
        ...
        case 'SET_APPEARANCE': {
            ...
            if (!cmd.appearance || typeof cmd.appearance !== 'object')
        ...
        case 'ACCEPT_ALLY':
        case 'DECLINE_ALLY': {
            if (!player.allianceOffers.includes(cmd.targetPlayerId))
                return fail('no_such_offer');
            return OK;
        }
        case 'BREAK_ALLY': {
            ...
            if (!target || target.id === playerId)
                return fail('unknown_player');
            if (!player.allies.includes(cmd.targetPlayerId))
                return fail('not_allied');
            return OK;
        }
        case 'TRADE_OFFER': {
        ...
        case 'TRADE_ACCEPT': {
            ...
            if (offer.toId !== playerId)
                return fail('not_recipient');
            if (!canAfford(player.resources, offer.want))
        ...
        case 'TRADE_DECLINE': {
            ...
            if (!offer)
                return fail('no_such_offer');
            if (offer.toId !== playerId && offer.fromId !== playerId)
                return fail('not_recipient');
            return OK;
        }
        case 'PING_MAP': {
            if (!state.territories[cmd.territoryId])
                return fail('unknown_territory');
            if (!['attack', 'defend', 'help'].includes(cmd.kind))
                return fail('malformed');
            return OK;
        }
        case 'CHAT': {
...
export function applyCommand(state, playerId, cmd) {
    ...
    switch (cmd.t) {
        ...
        case 'START_RESEARCH': {
            const tech = TECH_BY_ID[cmd.techId];
            const ticks = Math.max(1, Math.round(tech.researchTicks / player.modifiers.researchMul / state.config.speed));
            player.research = { techId: cmd.techId, completesAtTick: state.tick + ticks };
            return OK;
        }
        case 'SPEND_SKILL': {
            if (!spendSkillPoint(player.leader, cmd.branch))
                return fail('no_points');
        ...
        case 'TRADE_OFFER': {
            createTradeOffer(state, playerId, cmd.targetPlayerId, sanitizeDelta(cmd.give), sanitizeDelta(cmd.want));
            return OK;
        }
        case 'TRADE_ACCEPT': {
        ...
        case 'CHAT': {
            addEvent(state, 'chat', cmd.channel === 'team' ? playerId : null, cmd.text.slice(0, 200), { data: { from: playerId, channel: cmd.channel, team: player.team } });
            return OK;
        }
        default:
            return fail('unknown_command');
...
export class RateLimiter {
    ...
    tokens;
    lastRefillMs;
    constructor(capacity = COMMAND_RATE_LIMIT_PER_SECOND, refillPerSecond = COMMAND_RATE_LIMIT_PER_SECOND) {
