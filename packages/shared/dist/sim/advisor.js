...
import { areFriendly, armiesAt, armyUnitCount, canAfford, ownedTerritoryCount, playerTerritories, territoryDef, } from './state.js';
import { bestAvailableUnit, buildingBlocker, unitBlocker } from './construction.js';
import { UNITS } from '../config/units.js';
export function getSuggestions(state, playerId, limit = 3) {
    ...
    if (ownedTerritoryCount(state, playerId) > 2) {
        for (const army of Object.values(state.armies)) {
            if (army.ownerId !== playerId || army.movingTo !== null)
                continue;
            if (borderThreat(state, playerId, army.at) > 0)
                continue;
            const hasFrontier = (ADJACENCY[army.at] ?? []).some((l) => state.territories[l.to]?.ownerId !== playerId);
            if (hasFrontier)
                continue;
            out.push({
                kind: 'idle_army',
 ...
 */
function suggestBuilding(state, playerId) {
    const player = state.players[playerId];
    const mine = playerTerritories(state, playerId);
    const income = player.income;
    ...
    for (const { building } of ranked) {
        for (const territory of mine) {
            if (buildingBlocker(state, playerId, territory.id, building) !== null)
                continue;
...
function findTrainingSite(state, playerId, unit) {
    for (const territory of playerTerritories(state, playerId)) {
        if (unitBlocker(state, playerId, territory.id, unit, 1) === null)
            return territory.id;
    }
    return null;
}
function findNeutralBorder(state, playerId) {
    for (const army of Object.values(state.armies)) {
        if (army.ownerId !== playerId || army.movingTo !== null)
            continue;
        if (armyUnitCount(army) < 2)
            continue;
        for (const link of ADJACENCY[army.at] ?? []) {
            const target = state.territories[link.to];
            if (!target || target.ownerId !== null)
                continue;
            if (armiesAt(state, link.to).length > 0)
                continue;
...
function borderThreat(state, playerId, territoryId) {
    let threat = 0;
    for (const link of ADJACENCY[territoryId] ?? []) {
        for (const army of armiesAt(state, link.to)) {
            if (areFriendly(state, army.ownerId, playerId))
                continue;
...
function findWeakEnemyBorder(state, playerId) {
    for (const army of Object.values(state.armies)) {
        if (army.ownerId !== playerId || army.movingTo !== null)
            continue;
