...
import { BATTLE_POP_DAMAGE_PER_MINUTE, POP_BASE_GROWTH_PER_MINUTE, POP_FOOD_UPKEEP_PER_1K_PER_MINUTE, POP_INCOME_PER_1K_PER_MINUTE, POP_MAX_PER_SLOT, POP_STARVATION_RATE, TERRAIN_YIELD_MUL, TERRITORY_BASE_YIELD, UNREST_DECAY_PER_MINUTE, perTick, } from '../config/constants.js';
...
export function playerUpkeep(state, playerId) {
    const out = emptyResources();
...
export function computeIncome(state, player) {
    ...
    for (const key of RESOURCE_KEYS)
        gross[key] -= upkeep[key] * m.upkeepMul;
    return gross;
}
/**
...
export function applyPopulationTick(state, contested) {
    const speed = state.config.speed;
    for (const territory of Object.values(state.territories)) {
        const def = territoryDef(territory.id);
...
export function addDelta(target, delta, scale = 1) {
    for (const key of RESOURCE_KEYS) {
        const v = delta[key];
        if (v !== undefined)
            target[key] += v * scale;
