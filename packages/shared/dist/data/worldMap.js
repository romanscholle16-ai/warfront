 ...
 */
const T = (id, name, continent, lon, lat, terrain, basePop, baseEcon, resources, slots, coastal) => ({ id, name, continent, lon, lat, terrain, basePop, baseEcon, resources, slots, coastal });
const territories = [
    // ── North America ───────────────────────────────────────────────────────
...
export const TERRITORY_DEFS = Object.fromEntries(territories.map((t) => [t.id, t]));
...
function buildAdjacency(map) {
    ...
    for (const [a, b, kind] of map.edges) {
        ...
        if (!da || !db)
            throw new Error(`Map edge references unknown territory: ${a} <-> ${b}`);
...
export const ADJACENCY = buildAdjacency(EARTH_MODERN);
